...
function initFeatureRegistry() {
  ...
  // Setup Capacitor app lifecycle (no-op on web)
  try {
    const { app } = window.Capacitor ?? {};
    if (app) {
      app.addListener("pause", () => registry.broadcast("onPause"));
      app.addListener("resume", () => registry.broadcast("onResume"));
    }
...
function renderSidebar() {
  ...
  nav.innerHTML = "";
  const tabs = registry ? registry.getTabs() : [];
